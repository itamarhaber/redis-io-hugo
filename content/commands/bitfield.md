...
Usually [BITFIELD](/commands/bitfield) is a fast command, however note that addressing far bits of currently short strings will trigger an allocation that may be more costly than executing the command on bits already existing.

## Orders of bits

The representation used by [BITFIELD](/commands/bitfield) considers the bitmap as having the

...
As per Redis 6.2.0, this command is considered deprecated. Please prefer using the [ZRANGE](/commands/zrange) command with the `BYLEX` argument in new code.
...
Because of the first *normalized* part in every element (before the colon character), we are forcing a given comparison, however after the range is queries using [ZRANGEBYLEX](/commands/zrangebylex) the application can display to the user the second part of the string, after the colon.

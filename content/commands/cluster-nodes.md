...
[CLUSTER NODES](/commands/cluster-nodes) provides all this information, that is, the current cluster
...
[CLUSTER NODES](/commands/cluster-nodes), that provides more information, should be used for
...
of the node we are asking to generate [CLUSTER NODES](/commands/cluster-nodes) output. The other
...
Importing and migrating slots are emitted in the [CLUSTER NODES](/commands/cluster-nodes) output as follows:
...
Note that the format does not have any space, so [CLUSTER NODES](/commands/cluster-nodes) output format is plain CSV with space as separator even when this special slots are emitted. However a complete parser for the format should be able to handle them.

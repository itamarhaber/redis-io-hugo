...
1. The system administrator sends a [CLUSTER MEET](/commands/cluster-meet) command to force a node to meet another one.
...
So, if we link node A with node B via [CLUSTER MEET](/commands/cluster-meet), and B with C, A and C will find their ways to handshake and create a link.
...
Moreover [CLUSTER MEET](/commands/cluster-meet) does not need to be reciprocal. If I send the command to A in order to join B, I don't need to also send it to B in order to join A.

## Implementation details: MEET and PING packets

When a given node receives a [CLUSTER MEET](/commands/cluster-meet) message, the node specified in the

...
* [CLUSTER FAILOVER](/commands/cluster-failover), unless the **TAKEOVER** option is specified, does not execute a failover synchronously.
...
* A replica can only be promoted to a master if it is known as a replica by a majority of the masters in the cluster.
  ...
  To check that the masters are aware of a new replica, you can send [CLUSTER NODES](/commands/cluster-nodes) or [CLUSTER REPLICAS](/commands/cluster-replicas) to each of the master nodes and check that it appears as a replica, before sending [CLUSTER FAILOVER](/commands/cluster-failover) to the replica.
* To check that the failover has actually happened you can use [ROLE](/commands/role), `INFO REPLICATION` (which indicates "role:master" after successful failover), or [CLUSTER NODES](/commands/cluster-nodes) to verify that the state of the cluster has changed sometime after the command was sent.

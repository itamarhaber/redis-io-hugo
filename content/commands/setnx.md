...
[SETNX](/commands/setnx) is short for "**SET** if **N**ot e**X**ists".
...
That said, [SETNX](/commands/setnx) can be used, and was historically used, as a locking primitive. For example, to acquire the lock of the key `foo`, the client could try the
following:
...
If [SETNX](/commands/setnx) returns `1` the client acquired the lock, setting the `lock.foo` key
...
If [SETNX](/commands/setnx) returns `0` the key is already locked by some other client.
...
and then try to issue a [SETNX](/commands/setnx), as there is a race condition here, when
...
* C1 and C2 read `lock.foo` to check the timestamp, because they both received
  `0` after executing [SETNX](/commands/setnx), as the lock is still held by C3 that crashed

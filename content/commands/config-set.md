...
The list of configuration parameters supported by [CONFIG SET](/commands/config-set) can be obtained
...
All the configuration parameters set using [CONFIG SET](/commands/config-set) are immediately loaded
...
* In options where bytes or other quantities are specified, it is not
  ...
  greater, it is possible to use [CONFIG SET](/commands/config-set) with memory units for
...
(and the other way around) using the [CONFIG SET](/commands/config-set) command.

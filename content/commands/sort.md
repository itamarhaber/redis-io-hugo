...
This is [SORT](/commands/sort) in its simplest form:
...
`weight_3`, [SORT](/commands/sort) can be instructed to use these weights to sort `mylist` with
...
The `BY` option can also take a non-existent key, which causes [SORT](/commands/sort) to skip
...
By default, [SORT](/commands/sort) returns the sorted elements to the client.
...
of a [SORT](/commands/sort) operation can be cached for some time.
Other clients will use the cached list instead of calling [SORT](/commands/sort) for every
request.

...
The first argument of [EVAL](/commands/eval) is a Lua 5.1 script.
...
The second argument of [EVAL](/commands/eval) is the number of arguments that follows the script
...
error that in turn will force [EVAL](/commands/eval) to return an error to the command caller,
...
However it violates the [EVAL](/commands/eval) command semantics as all the keys that the script
...
keys the command will operate on.  In order for this to be true for [EVAL](/commands/eval), keys must be passed explicitly.
...
control what [EVAL](/commands/eval) will return to the client.
...
The [EVAL](/commands/eval) command forces you to send the script body again and again.
...
[EVALSHA](/commands/evalsha) works exactly like [EVAL](/commands/eval), but instead of having a script as the first
...
*   If the server does not remember a script with this SHA1 digest, a special
    error is returned telling the client to use [EVAL](/commands/eval) instead.
...
the hood even when the client actually calls [EVAL](/commands/eval), in the hope the script was
already seen by the server.
If the `NOSCRIPT` error is returned [EVAL](/commands/eval) will be used instead.

Passing keys and arguments as additional [EVAL](/commands/eval) arguments is also very useful in
...
of a Redis instance forever. This means that if an [EVAL](/commands/eval) is performed against a Redis instance all the subsequent [EVALSHA](/commands/evalsha) calls will succeed.
...
The only way to flush the script cache is by explicitly calling the [SCRIPT FLUSH](/commands/script-flush) command, which will _completely flush_ the scripts cache removing all the
...
unless an administrator explicitly called the [SCRIPT FLUSH](/commands/script-flush) command.
...
A common pattern is to call [SCRIPT LOAD](/commands/script-load) to load all the scripts that will
...
*   [SCRIPT KILL](/commands/script-kill)
...
It is possible to call [SELECT](/commands/select) inside Lua scripts like with normal clients,
...
so whatever the connection that is invoking the [EVAL](/commands/eval) or [EVALSHA](/commands/evalsha) command
...
* It starts accepting commands again from other clients, but will reply with a
  ...
  The only allowed commands in this status are [SCRIPT KILL](/commands/script-kill) and `SHUTDOWN
  NOSAVE`.
* It is possible to terminate a script that executes only read-only commands
  using the [SCRIPT KILL](/commands/script-kill) command.
...
*   Always use plain [EVAL](/commands/eval) when in the context of a pipeline.

*   Accumulate all the commands to send into the pipeline, then check for [EVAL](/commands/eval)
    commands and use the [SCRIPT EXISTS](/commands/script-exists) command to check if all the scripts are
    already defined.
    If not, add [SCRIPT LOAD](/commands/script-load) commands on top of the pipeline as required, and
    use [EVALSHA](/commands/evalsha) for all the [EVAL](/commands/eval) calls.
